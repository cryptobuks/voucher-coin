...
#if defined(NDEBUG)
# error "VoucherCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa91b9f99b2b847aeafe7617c83f109e5d834952ddaa4a55d5780a426232e9558");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // VoucherCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "VoucherCoin Signed Message:\n";
...
{
    // VoucherCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // VoucherCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // VoucherCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1530630336, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "RT 02 Jul 2018 Macron replaces French envoy to Hungary for praising";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b17fadd260b6f84654110ee41c05e4d2ed0a7c35f7dc269d4152d1b414298f481fce1afbb6886f19cc982c8780db2f352ae4a26cae6d0b46e42992d817c5a2f9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1530630336;
        ...
        assert(block.hashMerkleRoot == uint256("0x146dce3d29d693301bd7f498c26f75006189cc966d07087a133c135fd803e0d9"));
...
unsigned char pchMessageStart[4] = { 0x6f, 0xdf, 0x47, 0x22 }; // VoucherCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VoucherCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VoucherCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VoucherCoinMiner(CWallet *pwallet)
{
    printf("VoucherCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("vouchercoin-miner");
    ...
    try { loop {
        ...
        printf("Running VoucherCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("VoucherCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VoucherCoinMiner, pwallet));
}
